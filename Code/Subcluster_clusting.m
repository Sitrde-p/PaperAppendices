function [idxbest, MedoidsBest, sumDbest, Dbest, Midx, info] = Subcluster_clusting(X, k, varargin)
%   IDX = Subcluster_clusting(X, K) partitions the points in the N-by-P data matrix X
...
%   of X correspond to points, columns correspond to variables. Subcluster_clusting
%   returns an N-by-1 vector IDX containing the cluster indices of each
%   point.  By default, Subcluster_clusting uses squared
%   Euclidean distances.
%
%   treats NaNs as missing data, and ignores any rows of X that
%   contain NaNs.
%
%   [IDX, C] = Subcluster_clusting(X, K) returns the K cluster medoid locations in
%   the K-by-P matrix C.
%
%   [IDX, C, SUMD] = Subcluster_clusting(X, K) returns the within-cluster sums of
%   point-to-medoid distances in the K-by-1 vector SUMD.
%
%   [IDX, C, SUMD, D] = Subcluster_clusting(X, K) returns distances from each point
%   to every medoid in the N-by-K matrix D.
%
%   [IDX, C, SUMD, D, MIDX] = Subcluster_clusting(X, K) returns the indices MIDX such
%   that C = X(MIDX,:)
%
%   [IDX, C, SUMD, D, MIDX, INFO] = Subcluster_clusting(X, K) returns a structure INFO
%   with information about how the algorithm was executed.
%
%   [ ... ] = Subcluster_clusting(..., 'PARAM1',val1, 'PARAM2',val2, ...) specifies
%   optional parameter name/value pairs to control the iterative algorithm
%   used by Subcluster_clusting.  Parameters are:
%
...
%                            used by Subcluster_clusting.  If a parallel pool is
...
%                            invoking Subcluster_clusting with 'UseParallel' set to
%                            TRUE.
%
...
%                      proportional to its distance from the point抯
...
%                      you can pass in [] for K, and Subcluster_clusting infers K from
...
%       [idx, meds] = Subcluster_clusting(X, 2, 'Distance','city', 'Algorithm','small',...
...
hadNaNs = any(wasnan);
if hadNaNs
    warning(message('stats:MissingDataRemoved'));
    X = X(~wasnan,:);
end
...
if ischar(start)
    ...
    if length(j) > 1
        error(message('stats:AmbiguousStart', start));
    elseif isempty(j)
        error(message('stats:UnknownStart', start));
    elseif isempty(k)
        error(message('stats:MissingK'));
    end
    start = startNames{j};
elseif isnumeric(start)
    ...
    elseif reps ~= size(initialMedoids,3);
        error(message('stats:StartBadThirdDimSize'));
    end
    
else
    error(message('stats:InvalidStart'));
end
...
if ~(isscalar(k) && isnumeric(k) && isreal(k) && k > 0 && (round(k)==k))
    error(message('stats:InvalidK'));
    % elseif k == 1
    % this special case works automatically
elseif n < k
    error(message('stats:TooManyClusters'));
end
...
if ~isscalar(reps) || reps<1 || reps==inf || (uint64(reps) ~= reps)
    error(message('stats:ReplicatesType'));
end
...
if display > 1 % 'iter' or 'final'
    ...
    if usePool && display == 2 % 'iter' only
        warning(message('stats:displayParallel2'));
        ...
        if useParallel
            warning(message('stats:displayParallel'));
        end
...
if display > 0 % 'final' or 'iter'
    fprintf('%s\n',getString(message('stats:FinalSumOfDistances',sprintf('%g',totsumDbest))));
end
